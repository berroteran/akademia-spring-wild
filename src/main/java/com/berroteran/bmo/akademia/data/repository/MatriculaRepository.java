...
public interface MatriculaRepository extends CrudRepository<Matricula, Integer> {
    ...
    @Query("select m from Matricula m join fetch m.curso c join fetch m.sucursal s join fetch m.alumno a join fetch c.materia mat join fetch c.horario where m.activo = true")
